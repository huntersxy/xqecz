    ...
    <div class="w-[90%] max-w-[600px] bg-[var(--theme-surface)] rounded-xl shadow-2xl overflow-hidden relative z-[10000]">
      ...
      <div class="p-5">
        ...
        <div v-else class="mb-4">
          ...
          <input type="file" @change="$emit('handleFileChange', $event)" class="p-2 border border-gray-200 rounded-lg bg-[var(--theme-surface)] cursor-pointer" :accept="uploadForm.type === 'image' ? 'image/*' : 'video/*'" />
          <div v-if="filePreview" class="relative mt-3 rounded-lg overflow-hidden">
            ...
            <button type="button" @click="$emit('clearPreview')" class="absolute top-2 right-2 w-7 h-7 border-none border-[var(--theme-card-border)]0 text-white rounded-full cursor-pointer text-lg flex items-center justify-center hover:bg-[var(--theme-hover-bg)]0 transition-colors">×</button>
          </div>
        </div>
        ...
        <div class="mb-4">
          ...
          <div class="mt-2">
            <div class="flex flex-wrap gap-2">
              ...
              <button type="button" @click="$emit('addTag', 'upload')" class="w-8 h-8 flex items-center justify-center bg-[var(--theme-surface)] border-dashed border-black/20 rounded-md theme-text-secondary hover:border-blue-300 hover:text-[var(--theme-primary)] transition-all" title="添加标签">
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="flex justify-end gap-3 px-5 py-4 bg-[var(--theme-hover-bg)]">
        <button @click="$emit('close')" class="px-4 py-2 bg-[var(--theme-surface)] border border-[var(--theme-card-border)] rounded-lg text-sm theme-text hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" :disabled="isUploading">取消</button>
        <button @click="$emit('submit')" class="px-4 py-2 bg-[var(--theme-primary)] border border-[var(--theme-primary)] rounded-lg text-sm text-white hover:bg-[var(--theme-primary)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed" :disabled="isUploading || (uploadForm.type === 'video' && uploadForm.file && uploadForm.file.size > 15 * 1024 * 1024 && !agreeUpload)">
          {{ isUploading ? '上传中...' : '提交' }}
        </button>
      </div>
    </div>
