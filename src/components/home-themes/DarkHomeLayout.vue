...
<template>
  <div class="min-h-screen p-2 sm:p-5 flex justify-center">
    <div class="w-full max-w-[1200px] min-h-screen overflow-hidden theme-card rounded-xl sm:rounded-xl shadow-lg shadow-black/20 border transition-all duration-500 relative">
      ...
      <div class="relative z-1">
        ...
        <div class="p-3 sm:p-6">
          <div class="text-center mb-5 sm:mb-8">
            ...
            <a @click="goToEasterEgg" class="block w-fit mx-auto px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-gradient-to-r from-[#818cf8] to-[#c084fc] text-white no-underline cursor-pointer transition-all duration-300 text-sm sm:text-base font-medium text-center hover:scale-105 hover:shadow-lg hover:shadow-[#818cf8]/30">✨ 探索精彩</a>
          </div>
          ...
          <div class="mb-4 sm:mb-6">
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 max-w-[600px] mx-auto">
              <input
                ...
                class="flex-1 text-sm px-4 py-3 sm:py-2.5 min-h-[44px] sm:min-h-[auto] border theme-border rounded-lg theme-surface outline-none transition-all focus:border-(--theme-primary) focus:ring-3 focus:ring-(--theme-primary)/10 theme-text placeholder:text-[var(--theme-text-secondary)]/40"
              />
              <button @click="handleSearch" class="px-5 py-3 sm:py-2.5 min-h-[44px] sm:min-h-[auto] bg-gradient-to-r from-[#60a5fa] to-[#818cf8] text-white border-none rounded-lg text-sm font-medium cursor-pointer transition-all hover:brightness-90 whitespace-nowrap">搜索</button>
            </div>
          </div>
                  ...
                  <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4" :class="{ 'opacity-30 pointer-events-none transition-opacity': isRecommendLoading }">
                    ...
                    >
                      <div class="relative w-full pt-[75%] theme-placeholder-bg overflow-hidden">
                        ...
                        <div v-if="content.type === 'video'" class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 bg-black/60 rounded-full flex items-center justify-center z-[2]">
                    ...
                    >
                      <div class="relative w-full pt-[75%] theme-placeholder-bg overflow-hidden">
                        ...
                        <template v-if="content.type === 'video'">
                          <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 bg-black/60 rounded-full flex items-center justify-center z-[2]">
                            <svg class="w-5 h-5 text-white ml-0.5" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M8 5v14l11-7z" />
                            </svg>
                          </div>
                        ...
                        <template v-if="content.type === 'link'">
                          <div class="absolute top-2 right-2 w-8 h-8 bg-gradient-to-br from-[#818cf8] to-[#c084fc] rounded-full flex items-center justify-center z-[2]">
                        ...
                        <template v-if="content.type === 'text'">
                          <div class="absolute top-0 left-0 w-full h-full p-4 theme-surface flex items-center justify-center text-center z-[2]">
                      ...
                      <div class="p-3 sm:p-4">
                        ...
                        <div class="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-2.5 flex-wrap">
                          ...
                          <div class="flex gap-1 flex-wrap">
                            <span v-for="tag in content.tags" :key="tag" class="px-1.5 py-0.5 bg-(--theme-warning)/10 rounded text-xs text-(--theme-warning)">{{ tag }}</span>
                          </div>
                        </div>
