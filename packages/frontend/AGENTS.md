...
| 文件 | 触发条件 | 流程 |
|------|---------|------|
| `ci.yml` | push 到 `master` / PR | 两端 CI：server `go vet`+`go test`+静态构建、frontend type-check+vitest（覆盖率上传 Codecov） |
| `deploy.yml` | push 到 `master` / 手动 | 两端构建 → FTP 部署（Go 单二进制 + 前端 dist）→ 宝塔面板 API 重启服务（项目名 `xqecz2`） |
| `sonarcloud.yml` | push 到 `master` / PR | SonarCloud 静态分析（server Go 覆盖率 + 前端扫描） |
